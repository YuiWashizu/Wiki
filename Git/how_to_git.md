...
### Gitでの変更をローカルに反映させる（pull）
他の人が`git push`した結果を自分のローカルリポジトリに反映させるためには、
```
$ git pull origin master
```
を行うことで、自分のローカルリポジトリに反映させることができる. 最新の状態にできる.

### Gitからコードをローカルに落とす（clone）
既存のリポジトリのコピーを取得したい場合、コピーしたいリポジトリのアドレスを取得する.<br>
取得するには以下の方法がある
- コピーしたいリポジトリのページに行き、右上の「Clone or download」をクリックしてアドレスをコピー
そのアドレスを用いて、
```
$ git clone [address]
```
をすると、既存のリポジトリのコピーを取得できる. 
